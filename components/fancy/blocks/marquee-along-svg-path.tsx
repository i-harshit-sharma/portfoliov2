...
//           viewBox={viewBox}
//           preserveAspectRatio={preserveAspectRatio}
//           className="w-full h-full [stroke-dasharray:65] [stroke-dashoffset:65] group-hover:animate-[draw-path_0.6s_ease-in-out_forwards]"
...
const MarqueeAlongSvgPath = ({
  ...
  pathId,
  preserveAspectRatio = "xMidYMid meet",
  showPath = true, // Changed default to true so you can see it drawing
  // SVG defaults
  width = "100%",
  height = "100%",
  viewBox = "0 0 100 100",
  ...
  // ─── One-shot animation: freezes when first box hits 100% ───
  const spacing = 100 / items.length

  // CHANGED: totalRange is exactly 100. The animation will stop when the 
  // first item (which starts at 0) reaches exactly 100%. The other items 
  // will be evenly spaced out behind it.
  const totalRange = 100
  const duration = (totalRange / baseVelocity) * 1000 // ms

  // CHANGED: Drive the SVG drawing animation based on the movement of the first box
  const pathProgress = useTransform(baseOffset, [0, 100], [0, 1])

  const animationRef = useRef<number | null>(null)
  const startTimeRef = useRef<number | null>(null)

  useEffect(() => {
    // Reset on remount / prop change
    baseOffset.set(0)
    ...
    const tick = (timestamp: number) => {
      ...
      baseOffset.set(progress * totalRange)
  ...
  }, [baseVelocity, items.length, duration, totalRange, baseOffset, onComplete])
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          {/* CHANGED: Swapped standard <path> for <motion.path> to animate the line drawing */}
          <motion.path
            id={id}
            d={path}
            stroke={showPath ? "currentColor" : "none"}
            strokeWidth="2" // Added default stroke width so it's visible
            fill="none"
            ref={pathRef as React.RefObject<SVGPathElement>}
            style={{
              pathLength: pathProgress // This draws the line alongside the first box!
            }}
          />
        </svg>
        ...
        {items.map(({ child, repeatIndex, itemIndex, key }) => {
          const itemOffset = useTransform(baseOffset, (v) => {
            const startOffset = -(itemIndex * spacing)
            const raw = Math.min(startOffset + v, 100)
            const clamped = Math.max(raw, 0)
            const value = easing ? easing(clamped / 100) * 100 : clamped
            return `${value}%`
          })

          const visibility = useTransform(itemOffset, (v: string) =>
            parseFloat(v) > 0 ? "visible" : "hidden"
          )
